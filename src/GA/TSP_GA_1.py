...
if __name__ == "__main__":
    problem = tsplib95.load("data/TSP/eil51.tsp")
    print("Số thành phố:", problem.dimension)

    nodes = list(problem.get_nodes())
    coords = np.array([problem.node_coords[i] for i in nodes])  # tọa độ cho plot
    matrix = np.array([[problem.get_weight(i, j) for j in nodes] for i in nodes])
    ...
    plot_scores(history, title="GA run 1", filename="images/tournament_OXCrossover_SwapMutation.png")
    plot_tour(coords, best_path, best_cost, title="GA run 1", filename="images/bestTour_Tournament_OXCrossover_SwapMutation.png")
    ...
    plot_scores(history, title="GA run 1", filename="images/roulette_PMXCrossover_InversionMutation.png")
    plot_tour(coords, best_path, best_cost, title="GA run 1", filename="images/bestTour_Roulette_PMXCrossover_InversionMutation.png")